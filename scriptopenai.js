...
/* ===== /HERO-FIRST BOOTSTRAP ===== */
 ...
 // ========= CONFIG =========
    ...
    const MODEL = "gpt-4o";               // or "gpt-4-turbo" for faster responses
    ...
    analyzeBtn.addEventListener('click', async ()=>{
      ...
      const key = (ENABLE_API_KEY_INPUT ? (window.__APIKEY||'') : OPENAI_API_KEY).trim();
      if(!key){ 
        alert('OpenAI API key not set. Set OPENAI_API_KEY in the source, or enable the key input.'); 
        return; 
      }
...
• List possible non-diagnostic observations and educational insights
...
• Note any image quality issues or ambiguities
• Use a calm, clear tone and avoid definitive clinical claims
• Focus on educational value while emphasizing the need for professional medical evaluation

Remember: This is for educational purposes only and should never replace professional medical consultation.
        `.trim();

        const userInstruction = `
Analyze these medical images for educational insights only. Identify visible features, explain in lay terms, and suggest when professional evaluation is appropriate. Offer generally safe observations if relevant. If multiple images show different views/slices, compare them succinctly.
        `.trim();

        // Build content array with multiple images for OpenAI format
        ...
        state.images.forEach(img => {
          content.push({
            ...
            image_url: {
              url: img.dataUrl,
              detail: 'high' // Use 'high' for better medical image analysis
            }
          });
        });
          ...
          try {
            ...
            if (errorJson.error && errorJson.error.message) {
              ...
              if (errorJson.error.code === 'invalid_api_key') {
                ...
                errorMessage = 'API quota exceeded. Please check your OpenAI billing and usage.';
